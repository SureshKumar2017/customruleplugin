...
public class AvoidMethodDeclarationRule extends BaseTreeVisitor implements JavaFileScanner {
  ...
  public void visitMethod(MethodTree tree) {
    // All the code located before the call to the overridden method is executed before visiting the node
	 ...
	 ModifiersTree m =  tree.modifiers();
	 Iterator<ModifierTree> as = m.iterator();
	 while(as.hasNext()) {
		 ModifierKeywordTree asdas = (ModifierKeywordTree)as.next();
		 if("private".equalsIgnoreCase(asdas.modifier().name())) {
			 System.out.println(" Inside private ");
			 // Adds an issue by attaching it with the tree and the rule
			 context.reportIssue(this, tree, "Avoid declaring private methods (don't ask why)");
		 }
	 } 
	  System.out.println("@@@@@@@@@@");
