...
public class AvoidSuperClassRule extends IssuableSubscriptionVisitor {
  ...
  public void visitNode(Tree tree) {
    ...
    List<Tree> list = treeClazz.members();
    for(Tree tree2 : list) {
    	
    	if(tree2 instanceof VariableTree) {
    		System.out.println("ddsfdsf");
    		VariableTree impl = (VariableTree)tree2;
    		TypeTree type= impl.type();
    		if("RestTemplate".equalsIgnoreCase(type.toString())) {
    			System.out.println("=-=-=-==-=-------------->");
    			reportIssue(tree, String.format("Dont use RestTemplate instead using webclient", type));
    		}
    	}
